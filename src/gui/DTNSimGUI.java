...
import routing.util.DayEndComputation;
import ui.DTNSimUI;
...
public class DTNSimGUI extends DTNSimUI {
    ...
    protected void runSim() {
        ...
        double previousUpdateTime = SimClock.getTime();
        ...
        while (simTime < endTime && !simCancelled) {
            if (guiControls.isPaused()) {
				...
				double dayEndTime = 3600;
                if ((simTime - previousUpdateTime)>=dayEndTime && (simTime - previousUpdateTime)<=(dayEndTime+0.1)) {
                    ExecutorService executorService = Executors.newSingleThreadExecutor();
                    DayEndComputation computation = new DayEndComputation(simTime, previousUpdateTime, dayEndTime);
                    FutureTask futureTask = new FutureTask(computation);
                    executorService.execute(futureTask);
                    previousUpdateTime = simTime;
                }
            }
            this.update(false);
        }
