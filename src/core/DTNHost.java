...
import routing.util.ContactTimeAndDurationModel;
...
public class DTNHost implements Comparable<DTNHost> {
    ...
    private List<ContactTimeAndDurationModel> contactTimeAndDelays;

    private List<TimeSeriesModel> timeSeries;
    ...
    public DTNHost(List<MessageListener> msgLs,
        ...
        this.contactTimeAndDelays = new ArrayList<>();
...
//				((AugurRouter)(i.getHost().getRouter())).addTimeDuration(startTime,0d, i.getHost().toString());
                i.update();
            }
        }
        this.router.update();
    ...
    public List<ContactTimeAndDurationModel> getContactTimeAndDelays() {
        return contactTimeAndDelays;
    }

    public List<TimeSeriesModel> getTimeSeries() {
        return timeSeries;
    }

    public List<TimeSeriesModel> findByTimeAndHost(Double timeInstant, int otherHost) {
        return timeSeries.stream().filter(timeSeriesModel -> timeSeriesModel.getOtherHost() == otherHost
                && timeSeriesModel.getTimeInstant() >= timeInstant
                && timeSeriesModel.getTimeInstant() < (timeInstant + 1)).collect(Collectors.toList());
    }

    public void setTimeSeries(List<TimeSeriesModel> timeSeries) {
